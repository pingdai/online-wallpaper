...
func (log *Log) MarshalDefaults() {
	...
	if strings.ToLower(envFlag) == constants.ENV_TEST ||
		...
		strings.ToLower(envFlag) == constants.ENV_P2P ||
		strings.ToLower(envFlag) == constants.ENV_BB_BANK ||
		strings.ToLower(envFlag) == constants.ENV_BANK ||
		strings.ToLower(envFlag) == constants.ENV_PROD {
...
func (log *Log) Create() {
	...
	//logrus.AddHook(open_id.NewOpenIDHook())
	//logrus.AddHook(phone.NewPhoneHook())
	//logrus.AddHook(uin.NewUinHook())
	//logrus.AddHook(user_id.NewUserIDHook())
	logrus.AddHook(merchant_code.NewMerchantCodeHook())
